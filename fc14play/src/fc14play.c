/* Example program for interfacing with fc14play.
**
...
// set to true if you want fc14play to always render to WAV
...
#endif
{
	// put this in a thread so that it can be cancelled at any time by pressing a key (it can get stuck in a loop)
	fcRecordWAV(filename, WAVRenderFilename, WAVSongLoopTimes, audioFrequency, masterVolume, stereoSeparation);
...
int main(int argc, char *argv[])
{
	...
	// Initialize fc14 system
	if (!fcInit(audioFrequency, audioBufferSize, masterVolume, stereoSeparation))
	{
		fcClose();

		printf("Error initializing Future Composer replayer: ");
		switch (fcGetErrorCode())
		{
	...
	if (!fcLoad(filename))
	{
		...
		printf("Error loading Future Composer module: ");
		switch (fcGetErrorCode())
		{
			default: printf("Unknown error...\n"); break;
			...
			case ERR_NOT_AN_FC_MODULE:
				printf("This is not a Future Composer module!\n");
			break;
		}
	...
	if (!fcPlay())
	{
		...
		printf("Error playing fc module: ");
		switch (fcGetErrorCode())
		{
			default: printf("Unknown error...\n"); break;

			case ERR_SONG_NOT_LOADED:
				printf("No song was loaded! Did you call fcLoad()?\n");
			break;
		}
	}
	...
	int32_t songLen = numSequences / SEQ_SIZE;
	if (songLen < 1)
		songLen = 1;
	...
	while (programRunning)
	{
		...
		int32_t pos = (int32_t)Channel[0].seqPos / SEQ_SIZE;
		if (pos < 1)
			pos = 1;

		printf(" Pos: %03d/%03d - Row: %02d/64 - Speed: %d %s               \r",
			pos, songLen, Channel[0].patPos, repspd,
			audio.pause ? "(PAUSED)" : "");
...
{
	printf("Usage:\n");
	printf("  fc14play input_module [-f hz] [-m mixingvol] [-b buffersize]\n");
	printf("  fc14play input_module [-s percentage] [--render-to-wav] [-wloop loops]\n");
	printf("\n");
	printf("  Options:\n");
	printf("    input_module     Specifies the module file to load (.fc/.smod)\n");
