...
**
** This code has been crafted for use with fc14play.
** Usage outside of fc14play may be unstable or give unwanted results.
*/
...
#include <math.h>
#include "replayer.h" // tickReplayer()
...
static int8_t nullSample[65536];
...
bool paulaInit(int32_t audioFrequency)
{
	...
	const double dSamplesPerTick = audio.outputFreq / AMIGA_PAL_VBLANK_HZ;

	int32_t bufferSize = (int32_t)ceil(dSamplesPerTick);
	fMixBufferL = (float *)malloc(bufferSize * sizeof (float));
	fMixBufferR = (float *)malloc(bufferSize * sizeof (float));
